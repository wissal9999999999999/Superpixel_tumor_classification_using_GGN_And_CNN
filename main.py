...
def main(cnn_model_name = 'densenet', gnn_model = 'GCN', superpixel_number = 10, learning_rate = 0.001, batch_size = 64, epochs = 10, train = True, saved = True):#, pretrained = False
    ...
    if saved:
        ...
        if train:
            ...
            plt.xticks([0, 1], ['normal', 'tumor'], fontsize = 12)
            plt.yticks([0, 1], ['normal', 'tumor'], fontsize = 12)
            # plt.colorbar()
            # plt.show()
...
if __name__ == "__main__":
    ...
    # if not os.path.exists(f'{current_file_path}/liver_data'):
    #     print("please make sure that liver_data folder is in the same directory as main.py")
    #     print("expected liver_data folder with train and test folders inside")
    #     print("run dowload_data.py to download the data and extract it to the liver_data folder")
    #     sys.exit()
    ...
    if not os.path.exists(f'{current_file_path}/liver_data') and not saved:
        print("please make sure that liver_data folder is in the same directory as main.py")
        print("expected liver_data folder with train and test folders inside")
        print("run dowload_data.py to download the data and extract it to the liver_data folder")
        sys.exit()
