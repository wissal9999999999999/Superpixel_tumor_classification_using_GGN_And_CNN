...
# device = 'cpu'
data_dir = f"{current_file_path}/liver_data"
...
def read_data(data_dir):
    ...
    return train_dir, test_dir, val_dir, train_normal_dir, train_tumor_dir, test_normal_dir, test_tumor_dir, val_normal_dir, val_tumor_dir
...
def get_image_paths(data_dir):
    train_dir, test_dir, val_dir, train_normal_dir, train_tumor_dir, test_normal_dir, test_tumor_dir, val_normal_dir, val_tumor_dir = read_data(data_dir)
    train_normal_image_paths = [os.path.join(train_normal_dir, f) for f in os.listdir(train_normal_dir)]
    train_tumor_image_paths = [os.path.join(train_tumor_dir, f) for f in os.listdir(train_tumor_dir)]
    test_normal_image_paths = [os.path.join(test_normal_dir, f) for f in os.listdir(test_normal_dir)]
    test_tumor_image_paths = [os.path.join(test_tumor_dir, f) for f in os.listdir(test_tumor_dir)]
    val_normal_image_paths = [os.path.join(val_normal_dir, f) for f in os.listdir(val_normal_dir)]
    val_tumor_image_paths = [os.path.join(val_tumor_dir, f) for f in os.listdir(val_tumor_dir)]
    return [train_normal_image_paths, train_tumor_image_paths, test_normal_image_paths, test_tumor_image_paths, val_normal_image_paths, val_tumor_image_paths]


def load_image(tain_normal_path,train_tumor_path,test_normal_path,test_tumor_path,val_normal_path,val_tumor_path):
    train_normal_image = read_image(tain_normal_path)
    train_tumor_image = read_image(train_tumor_path)
    test_normal_image = read_image(test_normal_path)
    test_tumor_image = read_image(test_tumor_path)
    val_normal_image = read_image(val_normal_path)
    val_tumor_image = read_image(val_tumor_path)
    return train_normal_image, train_tumor_image, test_normal_image, test_tumor_image, val_normal_image, val_tumor_image


# def slic_segment():
#     train_normal_image_paths, train_tumor_image_paths, test_normal_image_paths, test_tumor_image_paths, val_normal_image_paths, val_tumor_image_paths = get_image_paths(data_dir)
#     train_normal_image, train_tumor_image, test_normal_image, test_tumor_image, val_normal_image, val_tumor_image = load_image(train_normal_image_paths[0],train_tumor_image_paths[0],test_normal_image_paths[0],test_tumor_image_paths[0],val_normal_image_paths[0],val_tumor_image_paths[0])
...
def run_for_one_folder(folder_path, n_segments = 10,model = 'densenet121'):
    for one in folder_path:
        ...
        ss = time.time()
        make_dirs(f"{current_file_path}/liver_data_graphs_{model}_sp_{n_segments}/{data_typ}/{class_name}")
        if image_name.split('.')[-2] + ".gpickle" in os.listdir(f"{current_file_path}/liver_data_graphs_{model}_sp_{n_segments}/{data_typ}/{class_name}"):
            continue
        print("="*100)
        print("done images", len(os.listdir(f"{current_file_path}/liver_data_graphs_{model}_sp_{n_segments}/{data_typ}/{class_name}")))
        print("total images present ",len(os.listdir(f"{current_file_path}/liver_data/{data_typ}/{class_name}")))
        ...
        print("="*100)
        nx.write_gpickle(G, f"{current_file_path}/liver_data_graphs_{model}_sp_{n_segments}/{data_typ}/{class_name}" + "/" + image_name.split('.')[-2] + ".gpickle")
...
def graph_preperation(n_segments = 10,model = 'dense121'):
    print("starting")
    # make_dirs(current_file_path+'/'+f'liver_data_graphs_{model}_sp_{n_segments}')
...
def dataloader(path , batchsize = 64,saved = True, sp = 100, model_name = "densenet121"):
    ...
    if not saved:
        ...
        train_tumor = load_all_from_one_folder(f"{path}/train/tumor",1)
        ...
        train_data_arr = train_normal + train_tumor
        test_data_arr = test_normal + test_tumor
        val_data_arr = val_normal + val_tumor
